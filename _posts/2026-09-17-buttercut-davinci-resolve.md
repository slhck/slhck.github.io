...
I don't particularly like nonlinear video editors, but over time I got used to handling Premiere Pro. Then, over time, Creative Cloud became too expensive to me. I since switched to [DaVinci Resolve](https://www.blackmagicdesign.com/products/davinciresolve) — its free version does everything I need, and I haven't felt the need for any “pro” features. Hey, I've even memorized some keyboard shortcuts, which help a lot during editing.

When I record videos of myself talking about something, I often struggle with the rough cut. I usually record several takes, and then it takes me a while to put everything together in the editor. Because I wanted to speed this up, I tested some LLM-based approaches, so here's what I learned.

The main focus for me is always the content. A good take should have good _words_, you know? Now, if every word in the footage has a timestamp, you can not only have an LLM analyze your transcript, but also describe the edit as text, and turn that text into a timeline!
...
I usually start by transcribing all of the original footage. First I tried `.vtt` subtitles, but such subtitle files (`.vtt` or `.srt`) only have one time range per phrase. For fine-grained video editing, you need precise millisecond accuracy for every word. Here are some options:
...
- The `mw` CLI from [MacWhisper](https://www.macwhisper.com/), which is paid. Its JSON output has millisecond `start` and `end` values in `.segments[].words[]`. JSON is already fully structured, so `--style transcript --group words` must not be added:
...
You do need to carefully check the transcript. The smaller models in use by those tools might have troubles understanding acronyms or domain-specific language jargon. I keep a list of corrections (for example, "ABC" → "IBC") and apply it at the end. I don't fix the transcripts by hand usually.

Another caveat is that some cameras let the audio track start later than the video. At least that's the case for my Android phone. So you always need to add the audio start offset to every word time. `ffprobe` shows this offset as the audio stream's `start_time`.
...
The result is an edit decision list that, for now, lives inside the model's context. For each (cut) segment, the model already knows the source file and the in- and out-points in milliseconds. Now it's just a matter of persisting that edit list as a file, e.g. a set of clips with in and out points:
...
At first, I had the agent write the list as a shell script. The script ran `ffmpeg -ss IN -t DURATION -i SOURCE` for each segment and joined the pieces manually, which was useful for watching, but I soon noticed I still had to refine things manually with keyboard and mouse.
...
Luckily DaVinci can import Final Cut Pro XML (FCPXML) files (which I had never heard about before), so… let the LLM turn that edit list into FCPXML! Some gotchas:
...
Here, I actually didn't invent anything new, I just used the [export code](https://github.com/barefootford/buttercut/blob/main/lib/buttercut/export.rb) from ButterCut again, since it can write timelines in various formats.

Then, in DaVinci, import the timeline with *File → Import → Timeline*. That's it. It's so nice to just have your edit appear to you on-screen.

## The fine edit

In DaVinci, I then perform input source audio normalization. I mark all audio clips (or select the leftmost, then press <kbd>Y</kbd>) and right-click to normalize.
...
Once you are done, create a *Quick Export* (from the edit window, it is in the top right).

![](/assets/images/buttercut-davinci-resolve/quick-export.avif)

## Subtitling

This is the part that is maybe a bit hacky — I know subtitle tools exist, and probably DaVinci can do this on its own, but I like to be more flexible in the transcription, which is where LLMs help as well.

Basically I have two styles for subtiles:
...
Let the LLM generate it based on a transcription of the quick export, again using a Whisper-based tool.
